...
## Claude performance bundle

The Claude game-development compatibility explainer is built from
`src/components/ClaudeGameDevExplainer.tsx` through Astro/Vite. React, Material UI,
Material UI Icons, Emotion, and Recharts are included in Cloudflare's local build
output instead of being fetched as an `esm.sh` module graph in the visitor's browser.
The route is generated by `src/pages/ai-gamedev-compatibility-claude.astro`.

## Local Async Python bundle

The `/async-python` route is built from `src/components/AsyncPythonExplainer.tsx` through Astro/Vite. React, Material UI, Emotion, and Material icons are bundled during the Cloudflare build rather than imported from `esm.sh` in the visitor's browser. The route no longer depends on Google Fonts or any runtime framework CDN.

## npm and npx beginner bundle

The `/npm-and-npx` route is built from `src/components/NpmNpxExplainer.tsx` through Astro/Vite. It is a dark-by-default, mobile-first introduction to Node.js, npm, npx, local and global packages, `package.json`, `package-lock.json`, `node_modules`, scripts, clean installs, and safe package execution.
